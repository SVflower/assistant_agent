...
- M11a 已完成：ToolDisplay 语义摘要、normal/verbose/quiet、统一终端脱敏、流式 Markdown、`/display` 与 `run --quiet`；Console renderer 按职责拆分，还清 D19，未修改内核 Loop。
- 405 个测试通过（2 个平台能力测试跳过），覆盖率 80%，ruff/mypy 全绿；7060 行生产 Python 源码 + 1366 行 eval 基础设施。
