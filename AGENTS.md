...
> 当前架构见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)，贡献流程见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
6. **状态文档同步**：更新 ROADMAP 里程碑表的状态标记 + 顶部"项目当前状态"块，以及本文件、`README.md` 的"当前状态"段。数字（测试数/覆盖率/源码行数/剩余技术债）用**实测**（`pytest -q`、`--cov`、`wc -l`），不凭记忆。里程碑历史小节里的旧数字是当时快照，不回改。
...
- M23-R2 Agent 侧已完成：Session schema/contract v2 权威公开消息 ledger、稳定 ID/time/reply，绑定源
  Session 的原子幂等 fork、Chart Artifact 深复制与 `run_id=null` 重绑定；compaction 不改写或泄漏
  ledger，Event v1/RunState v6 不变，未修改 Loop。API/Web 待按正式 handoff 接入。
- M27 已完成：`present_chart` 接受小模型友好草稿，保守推断缺失列类型并只允许一次定向修正；
  cloud/LM Studio 走同一路径，ChartSpecV1/Event/RunState 不变，未修改 Loop。
- M28 Agent 侧已完成：逐字节兼容 ChartSpecV1，新增受控 ChartSpecV2、中立 TabularDatasetV1、
  15 类高频普通图表、确定性 histogram/boxplot/percent/aggregate、多轴/多面板与白名单 overlay；
  Session schema/contract v3、RunState v7，Event v1 不变，未修改 Loop。API/Web 待按正式 handoff 接入。
- M30 Agent 侧已完成：Heatmap 新建路径固定 X/Y category axis，拒绝空数据、全 null value 和空白坐标；
  多面板重复坐标错误提供安全结构化纠错 metadata，一次修正额度按图表意图隔离并可恢复。公共契约仅
  additive 扩展可选 metadata，Event v1、Session contract/schema v3、RunState v7 不变，未修改 Loop。
...
部署边界归档于 `docs/archive/phase16/`，M22 稳定性收口归档于 `docs/archive/phase17/`，M23-R2
权威消息 ledger 与 Session fork 归档于 `docs/archive/phase18/`，M27 模型图表输入归档于
