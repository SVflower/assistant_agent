...
- M23-R1 Agent 侧已完成：Session schema v1 锁内迁移、自动/用户标题、metadata CAS、服务端
  catalog/search/HMAC cursor 和权威 last_run；所有 Session 写路径锁内 fresh merge，终态同步不覆盖
  并发 rename。Event v1/RunState v6 不变；整个 R1 待 API/Web 接入。
- 705 个测试通过（6 个平台能力测试跳过），覆盖率 84%，ruff/mypy、12/12 import-linter、
  scripted 19/19、recovery 4/4 全绿；19,405 行/129 文件生产 Python + 1,617 行 eval 基础设施；
