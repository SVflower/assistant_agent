...
状态：第一阶段（MVP→M5）与第二阶段（M6 可观测性、M6.5 运行时预算、M7 技能+MCP、M8 上下文进化）**均已完成**。
**本文档是第一阶段的设计快照，正文 1–9 节保留作历史设计记录、不随里程碑更新**——当前实际状态与架构以 [ROADMAP.md](ROADMAP.md)（里程碑与能力清单）、[README.md](README.md)（当前用法）和 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)（当前架构唯一事实源）为准。
最后更新：2026-07-14
