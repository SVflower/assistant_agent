...
**范围**：
- 把现有 `ToolContext.confirm`（y/n）升级为**多选**：允许 / 永远允许这类 / 拒绝并说明原因。
- 从"只有 shell"**泛化**：任何工具都能声明"我需要确认"（工具基类加声明点）。
- "永远允许"记住选择（按工具/类别），本会话内不再重复问。
- 拒绝时用户可附带一句说明，反馈给模型调整做法。
- **中断**：`esc to interrupt`，流式/长任务执行中可打断（键盘监听 + 循环中断信号）。

**架构影响**：工具基类 + `ToolContext` + `Console` 确认交互；中断涉及信号处理。不动 loop 控制流主体。

**风险**：中断的键盘监听跨平台（Windows/终端）行为差异，需实测。

**验收标准**：
1. 危险 shell 命令弹出多选（允许/永远允许/拒绝），选择后正确继续或中止
2. "永远允许"后同类操作本会话不再询问
3. 拒绝并附说明时，模型能据此换做法
4. 任意工具可声明需确认（用一个新工具或 mock 验证）
5. 长任务执行中可用 esc 中断，干净收尾不残留
6. 现有测试全绿 + 新增确认/中断测试
