...
> 最后更新：2026-07-14（M7a 技能系统完成；D7 升级为 main.py 越软线。剩 D5/D6/D7/D8/D9/D10）
...
| D7 | **main.py 越过行数软线** | `main.py`(329)、`tools/file_ops.py`(278) | 🟡 | M7a 技能接线（_discover_skills + 注入 + ChatContext 传参）把 main.py 从 298 推到 329，越过软线 300（仅警告、不阻断；离硬线 500 仍远）。评审判断：wiring 组（_setup 等）经模块级 `_interrupt` 全局与命令函数耦合，收尾期硬拆有风险，不为凑数拆内聚代码 | 触发信号：再膨胀（逼近 500）或做 M7b 又要加接线时——把 wiring（_build_client/_discover_skills/_setup/_run_streamed）抽到 `cli/setup.py`，`_interrupt` 一并迁移。file_ops 按"读/写/编辑"分组 |
