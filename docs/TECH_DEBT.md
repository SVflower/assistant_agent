...
> 最后更新：2026-07-18（M16 代码复审：内核未改、质量门全绿；本期未新增技术债。D11 中原
> `cli/setup.py` 装配已拆为公共 service runtime/builders，但既有大状态机和新服务编排仍按软线跟踪；
> `service/sessions.py` 行数按实测校正为 377。）
...
| D11 | **十一个文件越过行数软线** | `agent/context.py`(342)、`agent/loop.py`(466)、`agent/recovery.py`(471)、`config/schema.py`(352)、`main.py`(315)、`mcp/manager.py`(396)、`obs/logger.py`(361)、`service/runtime.py`(320)、`service/sessions.py`(377)、`tools/base.py`(381)、`tools/registry.py`(385) | 🟢 | 全部低于硬线 500；M16 已把旧 `cli/setup.py`(449) 收缩为 adapter，并把 Runtime 构建 helper 独立；Loop/Recovery 仍接近硬线，service/sessions 是本期新增编排状态机 | Loop/Recovery 新增职责前必须先拆；service/sessions 新增并发/池职责时先拆 facade；`mcp/manager.py` 增加健康/重连职责时先拆 discovery/policy；禁止放宽硬线 |
