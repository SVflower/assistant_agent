...
> 最后更新：2026-08-19（Performance Foundation P0 收敛纯 telemetry checkpoint 写放大，并增加
> Agent 自身分段归因；工具并发明确留给带 `parallel_safe` 元数据的 P1，不在本轮引入。未运行全量
> pytest/coverage，不新增技术债，现有 8 项。）
...
| D11 | ~~**十一个文件越过旧行数软线**~~ ✅ 已还清（M19） | `docs/ARCHITECTURE.md`、`.importlinter` | ✅ | 旧 300/500 机械门禁被概念所有权、依赖契约、C901 循环检查和 600 行非阻断评审替代；Runtime/Session/Run、Provider/Tool 与 adapter 职责已归位；当前越线模块均已在架构事实源逐项评审，教学注释增长不视为职责增长 | 行数不再单独形成债；首次越线、增长约 20% 或新增独立职责时按状态不变量、依赖与测试定位复审 |
...
| D20 | ~~**MCP stdio stderr 单次长运行缺少在线硬上限**~~ ✅ 已还清（当前阶段） | `integrations/mcp/transport.py` | ✅ | stdio stderr 通过独立管道持续 drain，诊断文件只保留最后 256 KiB；stdout 仍专用于 MCP JSON-RPC，不会被诊断输出污染；关闭时按 SDK transport → drain 的顺序回收 | 当前仅保留尾部，不提供完整 stderr 历史；如需更长诊断应使用 MCP Server 自身日志或外部日志系统 |
| D21 | ~~**MCP 调用期缺少连续失败熔断**~~ ✅ 已还清（当前阶段） | `integrations/mcp/manager.py`、`integrations/mcp/status.py` | ✅ | 同一 Runtime 内同一 Server 连续 3 次 transport/SDK 异常后进入 30 秒短时熔断；熔断期间不再发送请求，工具返回 `dependency_unavailable`，状态为 `degraded_connection` 且 `error_category=breaker`；成功调用清零计数；不自动重放、不跨 Runtime/重启持久化 | 后续可增强 `/mcp doctor` 的失败次数和剩余冷却展示；D20 的 stderr 在线硬上限仍独立处理 |
