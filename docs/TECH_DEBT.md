...
> 最后更新：2026-07-20（M23-R2 权威消息 ledger 与安全 Session fork：797 passed/10 skipped、覆盖率 84%，
> Ruff/format/mypy、12 条 import-linter、scripted 19/19、recovery 4/4 全绿；生产源码
> 20,921 行/132 文件。未新增技术债，D25 风险上升，现有 7 项。）
...
| D25 | **Session/Run 应用门面继续增长** | `application/runs.py` | 🟡 | M22 后已偏大，M23-R2 增加权威 ledger、Session snapshot 和绑定源 fork 后达到 1085 行；状态改写仍委托 RunCoordinator，但查询/映射/fork 已具备独立变化原因 | 下一里程碑优先提取无状态 snapshot/ledger 映射、fork 与 retry/reconcile Application 用例；不拆散 Iterator 结束/释放 lease/Session sync 不变量 |
