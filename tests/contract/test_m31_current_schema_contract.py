...
def test_breaking_service_contract_version_and_public_exports():
    assert contracts.AGENT_SERVICE_CONTRACT_VERSION == 2
    assert service.AGENT_SERVICE_CONTRACT_VERSION == 2
...
def test_run_store_rejects_v1_through_v6_without_writing(tmp_path):
...
@pytest.mark.parametrize("version", [None, 0, 1, 2, 4])
def test_session_store_rejects_non_v3_without_rewriting(tmp_path, version):
